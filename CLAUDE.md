...
## Layout proportions vs. copy length

The layout is tuned to the copy currently in `content/site.ts`, and a few rules exist because the copy changed underneath them:

- **The name appears once.** The eyebrow used to read `{name} · {availability}` while the headline was also the name — with the headline collapsed to a single word that read as a stutter. The eyebrow is now availability only, and `headline.lead` owns the name. If a longer headline ever comes back, revisit the eyebrow.
- **The headline is sized for one word**: `clamp(2.5rem, 10vw, 3.7rem)`, `max-width: 16ch`, tight tracking. At the old `1.9-2.6rem` a lone word looked lost above the subhead. A multi-line headline would want the old smaller size back.
- **`now.knownStack` grew from 2 entries to 11**, which inverted the balance of the Currently grid: the pills column is now the taller one. Columns went `1.3fr 1fr` (heatmap-biased) to `1fr 1.3fr`, `align-items` from `start` to `stretch` so the two cards share a height, and the cards center their content vertically so the short heatmap doesn't sit at the top of a tall box. Pills are also denser (smaller padding, tighter gap) to keep 11 of them from becoming five rows.
- **The Side Projects description is one short line** against a tall stat block, so `.top` is `align-items: center` rather than `flex-start` — top-aligning a 1-line paragraph against a 3-line stat reads as a mistake. The description is also bumped to `0.95rem`/`--ink` so it holds its side of the card instead of looking like a caption.
...
`components/FallingPlayer.tsx` — a 3D Minecraft player (WebGL) drops in at the left edge of the viewport, takes fall damage, looks around, then bhops all the way across and off the right edge. Desktop only (hidden under 860px) and hidden under `prefers-reduced-motion`, same as the other components' motion.
...
**Mobile and touch are excluded in JS, not just CSS.** The effect returns immediately unless `isSupported()` passes: `(min-width: 860px)` (mirrors the stylesheet's breakpoint), `(hover: hover) and (pointer: fine)`, and *not* `prefers-reduced-motion`. Hiding the stage in CSS alone was not enough — the component still built a `SkinViewer` (a live WebGL context), opened an `AudioContext`, and fetched and decoded three sound files on phones, and a tap on the card fires `mouseenter` on touch devices, so the full soundtrack would play for an animation nobody could see. Bailing before any of that means no WebGL, no audio graph, no network requests, and no listeners on a phone. The check runs once on mount; a desktop window resized narrow keeps working until reload, which is the deliberate trade for not re-initialising WebGL on every resize.
...
Screen-space movement is applied as a `translate3d` on the `.rig` wrapper from the sim's block coordinates — the rig is anchored `left` (not `right`), so `x` grows from the left edge across the full viewport, and the run ends at `exitDistance()` = viewport width plus one canvas width, so he is fully off-screen before the sim resets; body movement (limbs, head, turn) is 3D rotation on `viewer.playerObject`. `blockPx` converts between the two and is derived from the camera at runtime (`visibleUnits` from fov and camera distance, 16 units per block), so changing `zoom` or `fov` cannot silently desync the fall distance from what's on screen.
